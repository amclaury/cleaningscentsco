...
if(isset($_POST["type"]) && $_POST["type"]=='add' && $_POST["product_qty"]>0)
{
	...
	while($statement->fetch()){
        ...
        if(isset($_SESSION["cart_test"])){  //if session var already exist
            ...
            {
                unset($_SESSION["cart_test"][$new_product['product_code']]); //unset old array item
            }           
        }
        $_SESSION["cart_test"][$new_product['product_code']] = $new_product; //update or create product session with new item  
    } 
}
...
if(isset($_POST["product_qty"]) || isset($_POST["remove_code"]))
{
	...
	if(isset($_POST["product_qty"]) && is_array($_POST["product_qty"])){
		foreach($_POST["product_qty"] as $key => $value){
			if(is_numeric($value)){
				$_SESSION["cart_test"][$key]["product_qty"] = $value;
			}
		}
	}
